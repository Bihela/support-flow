...
async def lifespan(app: FastAPI):
    global monitors_running, background_tasks
    init_db()
    monitors_running = True
    
    # Start background tasks in the main event loop
    email_task = asyncio.create_task(email_monitor_loop())
    whatsapp_task = asyncio.create_task(whatsapp_monitor_loop())
    background_tasks.extend([email_task, whatsapp_task])
...
app = FastAPI(title="Support Engineer Knowledge Hub", lifespan=lifespan)
...
# Helper to run synchronous IMAP polling
def poll_emails_sync():
...
async def email_monitor_loop():
    while monitors_running:
        try:
            # Run blocking poll_emails_sync in a separate thread
            alerts = await asyncio.to_thread(poll_emails_sync)
            for alert in alerts:
                await broadcast_alert(alert)
        except asyncio.CancelledError:
            break
        except Exception as e:
            logger.warning(f"General error in email_monitor_loop: {e}")
        try:
...
def poll_emails():
    """Sync wrapper for tests to execute a single pass of poll_emails_sync"""
    import asyncio
    try:
        # If there's an event loop running, call poll_emails_sync directly
        alerts = poll_emails_sync()
        for alert in alerts:
            try:
        ...
        logger.warning(f"Error in poll_emails wrapper: {e}")

def poll_whatsapp():
    """Sync wrapper for tests to execute a single pass of poll_whatsapp_sync"""
    import asyncio
    try:
        alerts = poll_whatsapp_sync()
        for alert in alerts:
            try:
                loop = asyncio.get_running_loop()
                loop.create_task(broadcast_alert(alert))
        ...
        logger.warning(f"Error in poll_whatsapp wrapper: {e}")
...
async def whatsapp_monitor_loop():
    while monitors_running:
        try:
            # Run blocking poll_whatsapp_sync in a separate thread
            alerts = await asyncio.to_thread(poll_whatsapp_sync)
            for alert in alerts:
                await broadcast_alert(alert)
        except asyncio.CancelledError:
            break
        except Exception as e:
            logger.warning(f"General error in whatsapp_monitor_loop: {e}")
        try:
            await asyncio.sleep(5)
        except asyncio.CancelledError:
            break
